...
df1 <-
  cr_s3_df_lvlAdj_left %>%
  select(c(1, 10:25)) #%>%
  #slice(1:2000) # only the first 2000 rows
...
## Addressing a bug that the previous for loop creates (when 1 is actually the id number)
```{r}
# recode id column of lvl df
for (z in 1:nrow(lvl)) {
  if (lvl[z,1] == "id") {
      lvl[z,1] = "1"
    }
