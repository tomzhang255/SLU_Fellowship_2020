...
cr_full <- read_csv("./research/clash_royale/data_by_season/clash_royale_matches_season_15_2020-09-08_to_2020-10-07.csv")
...
f <- 
  as.formula(
    paste(
      ...
      paste(interactions[-1:-99], collapse = " + "), " + ",
      paste("battletime:", c(names(cr)[102:200], quad_level_terms), sep = "", collapse = " + ") # battle interact with levels and levels^2
    )
  )


# fit model
mod_4000_s15_lm_2 <- lm(formula = f, data = cr)

saveRDS(mod_4000_s15_lm_2, "mod_4000_s15_lm_2.rds")



# strip model - reduce size
library(strip)
lmc <- strip(mod_4000_s15_lm_2, keep = "predict")
saveRDS(lmc, "mod_4000_s15_lm_2_strip.rds")



# save df as well
write_csv(cr, "mod_4000_s15_lm_2_data.csv")
...
dfQuant <- tibble(
  predRounded = round(mod_4000_s15_lm_2$fitted.values / 100) * 100,
  resid = mod_4000_s15_lm_2$residuals
) %>%
