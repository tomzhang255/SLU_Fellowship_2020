...
rarity <- read_csv("cr_rarity_level_adjustments.csv")
cr_cards <- jsonlite::fromJSON("clash_royale_card_info.json")$cards %>% select(name, rarity)

# reading in all lines
cr_full <- read_csv("data_by_season/clash_royale_matches_season_1_2019-07-01_to_2019-08-05_left.csv")
...
#  as.formula(
#    paste(
#      paste("log(leftstartingtrophies) ~ "), # response
#      paste(names(cr)[-2], collapse =  " + "), " + ", # battletime and first level vars
#      paste(quad_level_terms, collapse =  " + ")#, " + ", # quadratic level vars
#      paste(interactions, collapse = " + ")
#    )
...
# fit model...not set to run
 #cr_model <- biglm::biglm(formula = f, data = cr)

#cr_4000 <- cr %>% filter(leftstartingtrophies >= 4000)

#cr_4000_model <- lm(f, data = cr_4000,  na.action = na.exclude)
#cr_4000_model_log <- lm(f2, data = cr_4000,  na.action = na.exclude)


#qplot(y = fitted(cr_4000_model), 
#      x = cr_4000$leftstartingtrophies, 
#      geom = "point") + 
#  geom_smooth(se = FALSE) +
#  geom_abline(slope = 1, intercept = 0, color = "green")
#
#qplot(y = exp(fitted(cr_4000_model_log)), 
#      x = cr_4000$leftstartingtrophies, 
#      geom = "point") + 
#  geom_smooth(se = FALSE) +
#  geom_abline(slope = 1, intercept = 0, color = "green")
...
#qplot(x = cr_4000$battletime, 
#      y = resid(cr_4000_model), 
#      geom = "point") + 
#  geom_smooth(se = FALSE)


#hist(cr_4000$leftstartingtrophies)
