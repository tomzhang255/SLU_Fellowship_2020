...
# put back together
cr <- cr %>%
  select(battletime, startingtrophies, trophychange) %>%
...
# convert trophy change to win/lose
cr <- mutate(cr, trophychange = if_else(trophychange >= 0, 1, 0))
...
f <- # trophies ~ battletime (day) + levels (first degree)
  as.formula(
    paste(
      ...
      paste("battletime:", c(names(cr)[102:199], quad_level_terms), sep = "", collapse = " + ") # battle interact with levels and levels^2
    )
  )
...
# fit model
logit_s11_winsor_43_60_int_quad_dayInt <- glm(formula = f, data = cr, family = "binomial")

saveRDS(logit_s11_winsor_43_60_int_quad_dayInt, file = "logit_s11_winsor_43_60_int_quad_dayInt.rds")

rm(logit_s11_winsor_43_60_int_quad_dayInt)
