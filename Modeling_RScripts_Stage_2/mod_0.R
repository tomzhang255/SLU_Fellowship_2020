...
cr_cards <- jsonlite::fromJSON("./research/CR_Project_Tom/clash_royale_card_info.json")$cards %>% select(name, rarity)
...
f_0 <- 
  as.formula(
    paste(
      ...
      paste(names(cr_0)[-1], collapse =  " + "), " + ",
      paste(paste('I(', names(cr_0)[95:187], '^2)', sep = ''), collapse = " + "), " + ",
...
mod_0 <- lm(formula = f_0, data = cr_0)

saveRDS(mod_0, "mod_0.rds")
