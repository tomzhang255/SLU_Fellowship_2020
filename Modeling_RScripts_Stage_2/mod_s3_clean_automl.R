...
cr <- cr_full %>%
  ...
  sample_n(tbl = ., size = 30000) %>% # random sample of 30k
...
# convert battletime -> day of the month / do not filter
cr <-
  cr %>%
...
# build X matrix
# dayOfTheMonth (named as battletime), cards, levels,

# no levels^2, no card interactions,
# no day interact with levels & levels^2
...
saveRDS(amlmodel, "mod_s3_clean_automl.rds")
