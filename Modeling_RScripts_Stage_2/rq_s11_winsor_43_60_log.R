...
f <- # trophies ~ battletime (day) + levels (first degree)
  as.formula(
    paste(
      paste("log(startingtrophies) ~ "), # response
...
# fit model
rq_s11_winsor_43_60_log <- rq(formula = f, data = cr, tau = c(0.05, 0.10, 0.25, 0.50, 0.75, 0.95))

saveRDS(rq_s11_winsor_43_60_log, file = "rq_s11_winsor_43_60_log.rds")

rm(rq_s11_winsor_43_60_log)
