...
## (1) Run mod_4000_s16_lm_2.R on HPCC

** if there are any new cards, just add another row in the RScript **

It should save 5 objects after you run it:
  1. mod_4000_s16_lm_2.rds - the original model (very large ~ 6 GB)
  2. mod_4000_s16_lm_2_strip.rds - same model but with reduced size for predictions
  3. mod_4000_s16_lm_2_data.csv - the data used for regression (don't need it but just in case)
...
## (2) update cr_allcard_profiles_s16.R
Run this file to get cr_allcard_profiles_s16.csv (for win rate plot)
...
## (4) the files needed to publish the app
 ...
 - cr_allcard_profiles_s16.csv (for win rate plot)
 - cr_cards_s16.csv (for card icon URLs, got it from CR API)
 ...
 - mod_4000_s16_lm_2_strip.rds (got it from running the model-fitting RScript)
